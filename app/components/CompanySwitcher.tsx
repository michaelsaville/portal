 ...
 * user has, with the same chip-tile shape per row, role on the right.
 *
 * Single-link users see a static chip with no chevron. Multi-link users
 * see the chevron + popover. Activates the underlying server action on
 * selection.
 */
export default function CompanySwitcher({ links, activeClientId }: Props) {
  ...
  function selectLink(clientId: string) {
  ...
  return (
    <div ref={wrapRef} className="relative">
      <form ref={formRef} action={switchClientAction} className="hidden">
        <input ref={idInputRef} type="hidden" name="clientId" defaultValue={active.clientId} />
      </form>
        ...
        <div className="absolute left-0 right-0 top-full z-30 mt-1 overflow-hidden rounded-md border border-stone-200 bg-white shadow-lg">
          <ul role="listbox" className="max-h-72 overflow-auto py-1">
            {links.map((l) => {
              ...
              const isActive = l.clientId === active.clientId
              return (
                <li key={l.clientId}>
                  <button
                    type="button"
                    onClick={() => selectLink(l.clientId)}
